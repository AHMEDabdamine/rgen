      ...
      {showSettings && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 no-print">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 animate-in fade-in zoom-in duration-200">
            <h3 className="text-xl font-bold text-slate-800 mb-4">إعدادات مفتاح API</h3>
            <p className="text-slate-600 text-sm mb-4">أدخل مفتاح Google Gemini API الخاص بك. يتم تخزينه محلياً في متصفحك فقط.</p>
            <input 
              type="password"
              placeholder="AIzaSy..."
              value={apiKey}
            ...
            <div className="flex gap-2">
              <Button onClick={() => saveApiKey(apiKey)} className="flex-1">حفظ</Button>
              <Button variant="outline" onClick={() => setShowSettings(false)}>إلغاء</Button>
            </div>
            <a href="https://aistudio.google.com/app/apikey" target="_blank" className="block text-center mt-4 text-xs text-indigo-600 hover:underline">الحصول على مفتاح مجاني من هنا</a>
          </div>
        </div>
      )}
      ...
      <main className="w-full max-w-4xl space-y-12">
        ...
        {error && (
          <div className="bg-rose-50 border border-rose-200 text-rose-700 p-4 rounded-xl flex items-center justify-between gap-3 no-print">
            ...
            {!apiKey && (
              <button onClick={() => setShowSettings(true)} className="text-indigo-600 font-bold underline">افتح الإعدادات</button>
            )}
          </div>
        )}
